...
<?php
	...
	$form = isset($_GET['form']) ? strip_tags(htmlentities($_GET['form'])) : 'json';
	...
	$facet = isset($_GET['facet']) ? strip_tags(htmlentities($_GET['facet'])) : null;
	
	// Set default value for results sorting
	$sort = isset($_GET['sort']) ? strip_tags(htmlentities($_GET['sort'])) : null;
...
?>
	<h2 class="result">Search for <strong><?php echo urldecode($q); ?></strong> (About <?php echo $totalItems; ?> matches)</h2>			
...
?>
			...
			src="<?php $thumbnail = !isset($item['pagemap']['cse_thumbnail'][0]['src']) ? './meta/img/thumbnail-default.png' : $item['pagemap']['cse_thumbnail'][0]['src']; 
			echo rawurldecode($thumbnail); ?>" /></a>
			</p>
			<p class="result-description">
			<a href="<?php echo $link; ?>"><?php echo $item['htmlTitle']; ?></a>
			<br />
			<strong><?php echo substr($link, 7); ?></strong>
			<br />
...
<?php
			if (!is_null($previous)) {
?>
			<a href="./index.php?q=<?php echo urlencode($_GET['q']);?><?php if (!is_null($facet)) echo '&amp;facet=' . $facet;?>&amp;totalItems=<?php echo $totalItems; ?>&amp;start=<?php echo $previous; ?>">Previous</a></li>
			<li>	
<?php
			}
			if (!is_null($next)) {
?>
			<a  href="./index.php?q=<?php echo urlencode($_GET['q']);?><?php if (!is_null($facet)) echo '&amp;facet=' . $facet;?>&amp;totalItems=<?php echo $totalItems; ?>&amp;start=<?php echo $next; ?>">Next</a>
<?php
			}
?>
			</li>
